...
module.exports = class AfkCommand extends BaseCommand {
    ...
    async run(client, msg) {
        if (msg.channel.id === process.env.MEMBERS_HOLIDAY_CHANNEL) {
            ...
            let startDate = await promtUserToEnterDate(msg, "Start").catch((e) => Promise.reject({message: e.message}));

            let endDate = await promtUserToEnterDate(msg, "End").catch((e) => Promise.reject({message: e.message}));

            await msg.channel.send("User: " + msg.author.tag + " is AFK \nStart Date: " + startDate + "\nEnd Date: " + endDate).catch(e => {
